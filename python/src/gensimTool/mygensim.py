...
def check(text, corpora_documents=None):
    """
    检查文档相似性
    :param text: 要检查的文档
    :param corpora_documents: 语料库，如果不给定，则加载语料库进行检查
    :return:
    """
    if text is None:
        print("doc is blank")
    ...
    # 生成字典和向量语料
    # '''
    if corpora_documents is not None:
    ...
    # 处理测试文本
    text_corpus = dictionary.doc2bow(text)  # 转换成bow向量
...
def add_documents(dictionary, documents, prune_at=None):
    dictionary.add_documents(documents, prune_at)
...
if __name__ == '__main__':
    ...
    for item_text in raw_documents:
        train(item_text)
    check(cut("南京江心洲污泥偷排”等污泥偷排或处置不当而造成的污染问题，不断被媒体曝光"))
